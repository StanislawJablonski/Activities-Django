...
function WeekCalendar() {
      ...
      .forEach((el, idx) =>
        events.push(
          ...
          >
            ...
            <p style={{ marginBottom: "0" }}>Prowadzący:</p>
            <p style={{ marginBottom: "0" }}>{el.leader}</p>
          </div>
        )
      );
    return (
  ...
  const getWeekDayNameAndNumber = (dayNumber) => {
    ...
    return (
      <>
        {monthdays[getWeekNumber()]?.[dayNumber].number}{" "}
        <font size="2">
          {windowsize.width >= 872
            ? daysNames[dayNumber]
            : daysNamesShort[dayNumber]}
        </font>
      </>
    );
  };
      ...
      >
        ...
        >
          <div className="bg-dark w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(0)}</div>
          </div>
          <div className="bg-dark w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(1)}</div>
          </div>
          <div className="bg-dark w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(2)}</div>
          </div>
          <div className="bg-dark w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(3)}</div>
          </div>
          <div className="bg-dark w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(4)}</div>
          </div>
          <div className="bg-danger w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(5)}</div>
          </div>
          <div className="bg-danger w-100 mx-auto text-white text-center row overflow-hidden">
            <div className="col my-auto">{getWeekDayNameAndNumber(6)}</div>
          </div>
        </Container>
      </div>
    );
  ...
  return (
    <>
      <Nav fill variant="tabs" defaultActiveKey="/dashboard/monthCalendar">
        ...
        <Nav.Item>
          <Nav.Link href="/dashboard/monthCalendar">Tydzień</Nav.Link>
        </Nav.Item>
        <Nav.Item>
          <Nav.Link eventKey="link-1" href="/dashboard/monthCalendar">
            Miesiąc
          </Nav.Link>
        </Nav.Item>
      </Nav>
